...
class MyInterestsBloc extends Bloc<MyInterestsEvent, MyInterestsState> {
  ...
  MyInterestsBloc(this._getMyInterestsUseCase, this._withdrawInterestUseCase)
  ...
  Future<void> _load(Emitter<MyInterestsState> emit) async {
    ...
    result.fold(
      ...
      (interests) => emit(state.copyWith(interests: interests, isLoading: false)),
    );
  }
  ...
  Future<void> _withdraw(
    ...
    result.fold(
      ...
      (_) {
        final updated =
            state.interests.where((r) => r.id != requestId).toList();
        emit(state.copyWith(
          withdrawingId: null,
          interests: updated,
